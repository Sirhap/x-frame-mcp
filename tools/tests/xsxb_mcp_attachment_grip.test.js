...
test("attachment tool keeps the rendered grip on the hand through scale and rotation", async (t) => {
  ...
  for (const [scale, rotation] of [
    ...
    const result = await f.service.call("xsxb_add_attachment", {
      file_path: f.weapon,
      hand,
      t: 0.2,
      scale,
    ...
    try {
      ...
      assert.ok(
        image.data[pixel] > 220 && image.data[pixel + 1] < 50,
        `grip marker must cover hand at scale ${scale}, rotation ${rotation}`,
      );
    } finally {
      if (render.tempDir) fs.rmSync(render.tempDir, { recursive: true, force: true });
    }
  }
});
...
test("below attachment persists a negative layer order and invalid batches do not copy files", async (t) => {
  ...
  await assert.rejects(
    f.service.call("xsxb_add_attachment", {
      file_path: another,
      frames: [
        { frame: 0, scale: 1 },
        { frame: 1, scale: 0 },

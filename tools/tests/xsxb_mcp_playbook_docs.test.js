...
test("export_gif catalog names preview.path for visual QA", () => {
  const tool = toolDefinitions().find((entry) => entry.name === "xsxb_export_gif");
  assert.ok(tool, "xsxb_export_gif must stay in the catalog");
  assert.match(tool.description, /preview\.path/);
});

...
本目录是 **x-frame-mcp** 的实现与安装入口。此仓库从 XSXB-Frame-Tuner 抽出 MCP 层，独立运行；Tuner 网页仍在原项目。
...
- `lib/`：MCP 运行所需的业务/算法副本（抠图、工程、Godot 同步等），不再 `require` 仓库其余部分
  改 MCP 能力时改本目录。`../tools/xsxb_mcp_*.js` 只是兼容转发，不要把逻辑写回 `tools/`。算法在 `lib/`。`xsxb_open_tuner` 通过 `XSXB_TUNER_ROOT` / `XSXB_ROOT` 找 Tuner 的 `server.js`（本仓不含网页）。改 `MCP_TOOL_NAMES` 后必须在 Cursor MCP 面板重载 `xsxb`，否则会话还是旧目录。

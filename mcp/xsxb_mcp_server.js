...
const INSTRUCTIONS = [
  ...
  "Still-place: xsxb_overlay_grid → xsxb_plan_place → xsxb_place_image with overlay_id. Do not mix still views with animation grid.cells. Do not OCR pixel coordinates.",
  "Smear: xsxb_plan_smear then a pixel crescent (polyline) or trail mesh only if the arc already matches.",
  ...
  "Report actual results. If a capability is missing or you must leave MCP, tell the user and raise it to XSXB-Frame-Tuner with the tool, arguments, expected result and actual result.",
].join(" ");

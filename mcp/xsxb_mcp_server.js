...
const INSTRUCTIONS = [
  ...
  "Walk-lock: xsxb_measure_frames → xsxb_register_clip (apply bakes about the feet) → xsxb_export_sheet normalize=feet|none, xsxb_export_overlay or xsxb_diff_frames; plant walk/attack with xsxb_plant_feet reference_animation_id: idle (omitted defaults to idle) at y=-1, not 0,0. Do not use xsxb_place_image or xsxb_add_attack_trail.",
  ...
  "Observation writes need basis_snapshot_id; still cells need overlay_id. Register/plant/estimate/reorganize/compress preview until apply or dry_run:false. Sync is opt-in. Godot handoff: xsxb_sync_godot immediately before xsxb_validate_for_godot (require_gameplay default). validate_for_godot qa=review or qa=warn is not a pass (scale drift). Diff qa=review means open the PNG.",

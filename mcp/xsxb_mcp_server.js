...
const INSTRUCTIONS = [
  ...
  "Walk-lock: xsxb_measure_frames → xsxb_register_clip (apply bakes about the feet) → xsxb_export_sheet normalize=feet|none, xsxb_export_overlay or xsxb_diff_frames; plant walk/attack with xsxb_plant_feet (idle default) at y=-1, not 0,0. Do not use xsxb_place_image or xsxb_add_attack_trail.",
  "Video-to-loop: import, xsxb_get_animation, xsxb_cutout (border_flood for white or black plates; inspect preview.path), xsxb_analyze, then xsxb_reorganize_frames applyOrder (not loop.recommended.order alone) with analyze observation.snapshotId as basis; non-empty order commits (omit order=preview), then export_gif. Do not export_sheet every candidate.",

...
对于 `in_place` 动画，恢复点会保存外部源 PNG，但恢复这些外部文件必须显式 `restore_external:true`。缺失 PNG 可从恢复点补回；损坏的快照 blob 会在任何恢复写入前拒绝。

成功修改或手动保存后，自动清理超过 `XSXB_AUTO_REVISION_LIMIT` 的旧 `pre_edit` 恢复点，默认保留最新 20 个。`0` 禁用清理；其他值必须是十进制正整数，配置无效时停止清理并给出警告。最新撤销点、调用者明确保护的恢复点、手动保存、恢复前的安全副本，以及缺少用途信息的旧版快照都不会被清理，因此总数可能超过上限。保存时尚未成功的修改不会触发淘汰；恢复失败会保留安全副本。清理结果包含删除的 id 和警告，清理失败不会把已经成功的操作报告为失败。只有归属于当前项目、目录与文件均无符号链接且内容符合快照格式的恢复点可被清理。PNG 目前仅在单个恢复点内去重，手动与安全版本仍需按实际需求管理空间。
